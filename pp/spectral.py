...
def compute_psd(
    theta: np.ndarray, mean_interval: float, k: float, aggregate: bool = False
    ...
    poles_residuals = [
        ...
        / (
            p
            * np.prod(p - np.array([pole for pole in poles_values if pole != p]))
    ...
    return SpectralAnalysis(
        frequencies, powers, poles, poles_comps_agg if aggregate else poles_comps,
    )
